...
class ali_params:
	def __init__(self):
		#self.class_freq = np.asarray([2.629, 1.262, 66.701, 24.269, 0.757, 0.119, 4.277])
		self.class_freq = np.asarray([93.267, 0.177, 0.085, 4.491, 1.634, 0.051, 0.008, 0.288])
		self.class_weights = -np.log(self.class_freq / 100.0)
		self.num_classes = len(self.class_freq) + 1
		'''
		self.color_map = [[255, 255, 255], #unlabeled (white)
						  [255, 255, 255], #Don't care(white)
			 			  [0, 130, 200], # cyclist (cyan)
			 			  [255, 225, 25],   # tricycle (yellow)
			 			  [255, 0, 0], 	  # smallMot (red)
			 			  [0, 0, 255],   # bigMot (blue)
			 			  [124, 152, 0], 	  # pedestrian (dark green)
			 			  [245, 130, 48],   # crowds (purple)
						  [128, 128, 128]] # unknown (grey)
		'''
		self.color_map = [[255, 255, 255],
						  [255, 255, 255], #unlabeled (white)
			 			  [255, 255, 255], # cyclist (grey)
			 			  [255, 255, 255],   # tricycle (yellow)
			 			  [255, 255, 255], 	  # smallMot (dark green)
			 			  [0, 255, 0],   # bigMot (light green)
			 			  [255, 255, 255], 	  # pedestrian (red)
			 			  [255, 255, 255],   # crowds (purple)
						  [255, 255, 255]] # unknown (grey)
